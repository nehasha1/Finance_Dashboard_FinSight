...
type Props = {
  value: string;
  onChange: (quarterKey: string) => void;
  options: Option[];
  label?: string;
  className?: string;
  id?: string;
...
export function QuarterSelect({ value, onChange, options, label, className, id }: Props) {
  const selectId = id ?? 'quarter-select';
  ...
  return (
    <div className={cn('flex flex-col gap-1.5', className)}>
      ...
      <div className="relative">
        <CalendarClock className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" aria-hidden />
        <select
          id={selectId}
          value={value}
